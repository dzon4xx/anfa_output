...
{	 
 ...
 void Modbus_write_coil::write_coils()
 {
	 ...
	 if ( frame->calculate_CRC(error_check_lo_index) == crc) // if the calculated crc matches the recieved crc continue
	 {
		 ...
		 unsigned int no_of_coils = ((data[POS_REQUEST_QUANTITY_OF_REG_HI] << 8) | data[POS_REQUEST_QUANTITY_OF_REG_LO]); // combine the number of register bytes
		 unsigned int max_data = starting_address + no_of_coils;

		 if (starting_address < no_of_coils) // check exception 2 ILLEGAL DATA ADDRESS
		 {
...
#endif
 {
	 ...
	 if(frame->is_new)
	 {
		 if (in_byte!=frame->address)
		 {
			 frame->is_new = false;		//TODO: test przyspieszenia obslugi modbusa. Jesli pierwszy bajt nie jest twoim adresem nie analizuj komunikatu
		 }
